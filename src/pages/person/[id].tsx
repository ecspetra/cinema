...
import {faCalendarCheck, faFlag, faUser} from "@fortawesome/free-solid-svg-icons"
		...
		<MainLayout>
			<div className="w-full flex gap-x-7 py-7">
				<div className="w-full max-w-[340px]">
					<Image src={`https://image.tmdb.org/t/p/w440_and_h660_face${person.profile_path}`} defaultImage={defaultPersonImage} />
				</div>
				<div className="w-full">
					<Title className="text-7xl">{person.name}</Title>
					<Title variant="h2" className="mb-5 text-slate-400">{person.known_for_department}</Title>
					<div className="mb-5">
						...
						<div className="flex items-center text-sm">
							...
							<span>
								...
								{person.deathday && <span> — Date of death: {moment(person.deathday).format('MM.DD.YYYY')}</span>}
							</span>
						</div>
						<div className="flex items-center text-sm">
					...
					<Button className="mt-auto max-w-[200px]" onClick={test}>Add to favorites</Button>
				</div>
			</div>
		</MainLayout>
	)
